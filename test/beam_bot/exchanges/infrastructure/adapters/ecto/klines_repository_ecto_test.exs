defmodule BeamBot.Exchanges.Infrastructure.Ecto.KlinesRepositoryEctoTest do
  ...
  setup do
    ...
    klines = [
      %Kline{
        ...
        quote_volume: Decimal.new("5_000_000.0"),
        trades_count: 1000,
        taker_buy_base_volume: Decimal.new("50.0"),
        taker_buy_quote_volume: Decimal.new("2_500_000.0"),
      ...
      %Kline{
        ...
        open: Decimal.new("49_500.0"),
        high: Decimal.new("50_500.0"),
        low: Decimal.new("49_000.0"),
        close: Decimal.new("50_000.0"),
        volume: Decimal.new("120.0"),
        close_time: now,
        quote_volume: Decimal.new("5_940_000.0"),
        trades_count: 1200,
        taker_buy_base_volume: Decimal.new("60.0"),
        taker_buy_quote_volume: Decimal.new("2_970_000.0"),
        ignore: Decimal.new("0")
      }
    ]

    {:ok, count} = KlinesRepositoryEcto.store_klines(klines)
  ...
  describe "store_klines/1" do
    ...
    test "handles string decimal values" do
      ...
      assert {:ok, 1} = KlinesRepositoryEcto.store_klines([kline])
    end
    ...
    test "handles conflict with existing data", %{klines: [kline | _]} do
      ...
      assert {:ok, 0} = KlinesRepositoryEcto.store_klines([kline])
    end
  end
  ...
  describe "get_klines/5" do
    test "retrieves klines with all fields", %{klines: [kline | _]} do
      assert {:ok, [retrieved_kline | _]} =
               KlinesRepositoryEcto.get_klines(kline.symbol, kline.interval)
    ...
    test "respects limit parameter", %{klines: [kline | _]} do
      assert {:ok, retrieved_klines} =
               KlinesRepositoryEcto.get_klines(kline.symbol, kline.interval, 1)
  ...
  describe "get_latest_kline/2" do
    test "retrieves the latest kline", %{klines: [latest | _]} do
      assert {:ok, retrieved_kline} =
               KlinesRepositoryEcto.get_latest_kline(latest.symbol, latest.interval)
  ...
  describe "delete_klines/4" do
    test "deletes klines within time range", %{klines: [kline | _], now: now} do
      ...
      assert {:ok, []} = KlinesRepositoryEcto.get_klines(kline.symbol, kline.interval)
    end
    ...
    test "respects time range boundaries", %{klines: [kline | _], now: now} do
      ...
      assert {:ok, [_, _]} = KlinesRepositoryEcto.get_klines(kline.symbol, kline.interval)
    end
  end
end
